...
bool create_descriptor_set(VulkanFrameData *frame_data, VulkanSetupInfo *setup_info) {
	...
	auto res = vkAllocateDescriptorSets(frame_data->logical_dev, &alloc_info, frame_data->descriptor_sets);
	...
	for (size_t i = 0; i < MAX_CONCURRENT_FRAMES; ++i) {
		...
		db_info.buffer                 = frame_data->uniform_buff[i];
		db_info.offset                 = 0;
		...
		desc_write[0].dstSet               = frame_data->descriptor_sets[i];
		...
		desc_write[1].dstSet               = frame_data->descriptor_sets[i];
...
bool destroy_sync_objects(VulkanFrameData *frame_data) {
	...
	for (size_t i = 0; i < MAX_CONCURRENT_FRAMES; ++i) {
		...
		vkDestroySemaphore(frame_data->logical_dev, frame_data->render_finished[i], nullptr);
		vkDestroySemaphore(frame_data->logical_dev, frame_data->image_available[i], nullptr);
	}
...
bool create_uniform_buffer(VulkanFrameData *frame_data, VulkanSetupInfo *setup_info) {
	...
	for (size_t i = 0; i < MAX_CONCURRENT_FRAMES; i++) {
		create_buffer(setup_info, frame_data->physical_dev, sz, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &frame_data->uniform_buff[i], &frame_data->uniform_buff_mem[i]);

		vkMapMemory(frame_data->logical_dev, frame_data->uniform_buff_mem[i], 0, sz, 0, &frame_data->uniform_buff_mapped[i]);
	}
...
bool destroy_uniform_buffer(VulkanFrameData *frame_data) {
	...
	for (size_t i = 0; i < MAX_CONCURRENT_FRAMES; i++) {
		vkDestroyBuffer(frame_data->logical_dev, frame_data->uniform_buff[i], nullptr);
		vkFreeMemory(frame_data->logical_dev, frame_data->uniform_buff_mem[i], nullptr);
	}
